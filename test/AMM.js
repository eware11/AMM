...
describe('AMM', () => {
  ...
  describe('Swapping tokens', async () => {
    ...
    balance = await token2.balnceOf(investor1.address)
    console.log(`Investor1 Token2 balance before swap: ${ethers.utils.formatEther(balance)}`)

    // Estimate amount of tokens investor wil receive after swapping token1: include slippage
    ...
    balance = await token2.balanceOf(investor1.address)
    console.log(`Investor1 Toke2 balance before swap: $ehters.utils.formatEther(balance)} \n`)

    // Estimate amount of tokens investor1 will receive after swapping token1: includes slippage
    estimate = await amm.calculateTkoen1Swap(tokens(1))
    console.log(`Token2 Amount investor1 will revieve after swap: $(ethers.utils.formatEther(estimate)}`)
 ...
 // Investor 1 swaps a large amount
    ...
    console.log(`Investor2 Token1 balance before swap: ${thers.utils.formatEther(balance)}`)
    ...
    console.log(`Token1 Amount investor2 will receive after swap: ${ethers.utils.formatEther(estiamte)}`)
    ...
    await transaction.wait()
     ...
     expect(await token1.balanceOf(amm.address)).to.equal(await amm.tokenBalance())
     expect(await token2.balanceOf(amm.address)).to.equal(await amm.tokenBalance())
     ...
     expect(await amm.shares(deployer.address)).to.equal(0)
